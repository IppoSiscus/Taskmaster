...
import { LayoutDashboard, CheckSquare, Folder, Tag, Calendar, Plus } from 'lucide-react';
...
const Sidebar: React.FC = () => {
  ...
  const { isSidebarOpen, openProjectModal } = appContext;
      ...
      <nav className="flex-1 flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          {mainNavItems.map((item) => (
            <NavLink key={item.to} to={item.to} end={item.to === '/'} className={({ isActive }) => `${baseLinkClasses} ${isActive ? activeLinkClasses : ''}`}>
              {item.icon}
              {item.text}
            </NavLink>
          ))}
        ...
        <div className="flex-1">
            <div className="flex justify-between items-center px-3 mb-2">
                <h2 className="font-semibold text-lg">Progetti</h2>
                <button onClick={() => openProjectModal()} className="p-1 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700">
                    <Plus size={18} />
                </button>
            </div>
            <ProjectTree parentId={null} />
        </div>

        <div className="flex flex-col gap-1">
            {secondaryNavItems.map((item) => (
                <NavLink key={item.to} to={item.to} className={({ isActive }) => `${baseLinkClasses} ${isActive ? activeLinkClasses : ''}`}>
                {item.icon}
                {item.text}
                </NavLink>
            ))}
